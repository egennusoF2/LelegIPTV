...
| Settings playlists | `src/pages/settings.astro`, login flow | Functional | Native persists multiple Xtream profiles, shows saved list names, switches active list, removes lists, refreshes active list on request, clears the form after save, supports shortcut codes (`LELEG/JOLLY/AMICO`, `ALESSANDRO/GIORDANO`), and caches catalog data for 24 hours. Missing import/export, M3U/local playlists, and full account metadata parity. |
